...
it("should throw on invalid input", () => {
  expect(() => validateKey({ type: "", id: "test" })).toThrow("type must be a non-empty string");
});

it("should handle async errors", async () => {
  await expect(store.get({ type: "../bad", id: "test" })).rejects.toThrow(
...
it("should invalidate cache on write", async () => {
  ...
  // Update document
  await store.put({ type: "task", id: "1" }, { type: "task", id: "1", title: "Updated" });
